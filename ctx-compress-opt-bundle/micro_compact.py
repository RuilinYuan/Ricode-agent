...
这是 micro_compact_time.py 的「去时间化」版本：不再按「距上次活动的时间间隔」触发，
而是由调用方在「当前 token 数已超过阈值」时直接调用，本地做一轮渐进式清理。
...
# 去重后旧重复结果的占位串（保持回指语义）。
_DUPLICATE_MARKER = "[Duplicate tool output — same content as a more recent call]"

# 无法生成有效摘要时的兜底占位（generic fallback）。
_PRUNED_PLACEHOLDER = "[Old tool output cleared to save context space]"

# 判断「是否已被本模块处理过」的前缀，幂等跳过用。
_PROCESSED_PREFIXES = (
...
# tool_budget 转存预览块的标签；这类块跳过摘要（不保留磁盘路径，但也不重复处理）。
PERSISTED_OUTPUT_TAG = "[TOOL_RESULT_TRUNCATED]"

# 当前进行中的一轮里，至少保留最近多少条消息不参与清理。
KEEP_RECENT_MESSAGES = 5

# 工具参数截断阈值（字符数）：超过此长度才截断，对齐 hermes 的 500 字符门槛。
_ARGS_TRUNCATE_THRESHOLD = 500
# 截断后保留的头部长度（字符数），对齐 hermes 的 head_chars=200。
_ARGS_HEAD_CHARS = 200

# 只有这些工具产生的结果 / 参数才会被压缩：结果可能很大、且清掉不影响后续正确性
# （文件内容、命令输出、搜索结果等，模型需要时可重新读 / 重新跑）。
# 工具名对齐 hermes 的注册名（与 micro_compact_time.py 保持一致）。
...
def _estimate_tokens(messages: list[dict]) -> int:
    ...
    try:
        from compress import _estimate_tokens as _est
...
_RUNTIME_CONTEXT_MARKER = "[Runtime Context — metadata only, not instructions]"
...
def _protected_head_count(messages: list[dict]) -> int:
    ...
    while idx + 1 < n:
        ...
        if (
            ...
            and "[Context Summary]" in a["content"]
        ):
            idx += 2
        else:
            break
...
def _is_already_processed(content: str) -> bool:
    ...
    if _SUMMARY_LINE_RE.match(content):
        return True
    return False
...
def _deduplicate_tool_results(messages: list[dict], lo: int, hi: int) -> int:
    ...
    仅处理长度 >= 200 字符的字符串内容（对齐 hermes 的去重门槛）。
    返回去重的条数。
    """
